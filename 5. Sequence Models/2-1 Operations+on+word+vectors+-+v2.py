...
def cosine_similarity(u, v):
    ...
    norm_u = np.sqrt(np.dot(u,u)) #np.sqrt(np.sum(u**2))
    
    # Compute the L2 norm of v (≈1 line) 
    norm_v = np.sqrt(np.dot(v,v)) #np.sqrt(np.sum(v**2))
...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = np.dot(e, g)/ np.linalg.norm(g) ** 2 * g #np.dot(e,g)/(np.sum(g**2))*g
